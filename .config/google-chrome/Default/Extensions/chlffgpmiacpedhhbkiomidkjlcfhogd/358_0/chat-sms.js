...
var renderSmsChunks = function(thread, chunks) {
    ...
    chunks.forEach(function(chunk) {
        ...
        chunk.forEach(function(message) {
            ...
            contents.title = message.timestamp ? moment(Math.floor(message.timestamp * 1000)).format('llll') : ''
            ...
            if (message == lastMessage) {
                if (message.direction == 'incoming' && thread.recipients.length > 1 && 'recipient_index' in message) {
                    var recipient = thread.recipients[message.recipient_index]
                    var name = document.createElement('div')
                    name.className = 'chat-date'
                    name.textContent = recipient.name
                    name.style.marginLeft = '50px'
                    chunkHolder.appendChild(name);
                }
            }
